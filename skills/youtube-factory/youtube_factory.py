...
def generate_video(topic: str, style: str = "documentary", length: int = 8,
                   ...
                   output_name: str = None, script_file: str = None) -> dict:
    ...
    # Step 1: Script
    print("\nStep 1/6: Loading script...")
    if script_file and os.path.exists(script_file):
        print(f"   Using pre-written script: {script_file}")
        script = Path(script_file).read_text()
        # Copy script to output dir if not already there
        script_path = video_dir / "script.md"
        if str(script_path) != str(Path(script_file).resolve()):
...
def main():
    ...
    parser.add_argument("--script", help="Path to pre-written script file (skips script generation)")
